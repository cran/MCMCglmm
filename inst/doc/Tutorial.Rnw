...
\end{center}
\caption{MCMC output from  \texttt{model1} (black trace) which was fitted without a prior on the variance components (n=0) and \texttt{model2} in which a weak but proper prior (n=1, V=0.02) was used (red trace) }
\label{model2VCV-fig}
\end{figure}

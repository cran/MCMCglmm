...
double dcutpoints(const cs *liab, double *yP, int *observed, int start,int finish, double *oldcutpoints, double *newcutpoints, int stcutpoints, int ncutpoints, double sdcp, double sdl)
{
    ...
    for (j = 2 ; j < (ncutpoints-2); j++){
        llik += log(pnorm((oldcutpoints[stcutpoints+j+1]-oldcutpoints[j])/sdcp, 0.0, sdl, TRUE,FALSE)-pnorm((newcutpoints[stcutpoints+j-1]-oldcutpoints[j])/sdcp, 0.0, sdl, TRUE,FALSE));
        llik -= log(pnorm((newcutpoints[stcutpoints+j+1]-newcutpoints[j])/sdcp, 0.0, sdl, TRUE,FALSE)-pnorm((oldcutpoints[stcutpoints+j-1]-newcutpoints[j])/sdcp, 0.0, sdl, TRUE,FALSE));
    }

   llik += log(1.0-pnorm((newcutpoints[stcutpoints+ncutpoints-3]-oldcutpoints[stcutpoints+ncutpoints-2])/sdcp, 0.0, sdl, TRUE,FALSE));
   llik -= log(1.0-pnorm((oldcutpoints[stcutpoints+ncutpoints-3]-newcutpoints[stcutpoints+ncutpoints-2])/sdcp, 0.0, sdl, TRUE,FALSE));

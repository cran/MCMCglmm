...
\end{Sinput}
\begin{Soutput}
[1]  0.9060817  0.4155958 -0.1951143 -0.8125874 -0.4257024
\end{Soutput}
\end{Schunk}
...
Deviance Residuals: 
      1        2        3        4        5  
 0.9284   0.4379  -0.1728  -0.7902  -0.4034  

Coefficients:
            Estimate Std. Error t value Pr(>|t|)
(Intercept) -0.02235    0.30584  -0.073    0.945

(Dispersion parameter for gaussian family taken to be 0.4676994)

    Null deviance: 1.8708  on 4  degrees of freedom
Residual deviance: 1.8708  on 4  degrees of freedom
AIC: 13.274
...
Here we see that although the estimate of the mean (intercept) is the same, the estimate of the variance (the dispersion parameter: 0.468) is higher when fitting the model using \texttt{glm}. In fact the ML estimate is a factor of $\frac{n}{n-1}$ smaller.
...
+         log(dinvgamma(par[2], shape = priorR$nu/2, scale = priorR$nu/(2 * 
+             priorR$V)))
+ }
\end{Sinput}
\end{Schunk}
...
\begin{Schunk}
\begin{Sinput}
> prop.table(table(m1a.2$Sol > -2 & m1a.2$Sol < 2 & m1a.2$VCV < 
+     5))
\end{Sinput}
\begin{Soutput}
 FALSE   TRUE 
...
There is Monte Carlo error in the answer (0.983) but if we collect a large number of samples then this can be minimised.\\ 

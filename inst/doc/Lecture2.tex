...
\title{Generalised Linear Mixed Models (GLMM)}
...
The prior specification now needs to include an additional element \texttt{G}, which stands for ${\bf G}$-structure. Because multiple terms may be entered into the random formula,  \texttt{G} itself should be a list with each element defining a prior for each  term in the random formula. In this case we only have a single random term so \texttt{G} only has a single element. The name of the element is arbitrary and it may be useful to give it a meaningful name. However, I've just called it  \texttt{G1}:\\
...
Should we worry? Not really. We just have to be careful about how we express the results. Stating that the family variance is 0.776 is meaningless without putting it in the context of the assumed residual variance. It is therefore more appropriate to report the intraclass correlation which in this context is the expected correlation between the state Pupated/Not Pupated, for members of the same family. It can be calculated as:
...
Deviance Residuals: 
       Min          1Q      Median          3Q         Max  
-1.212e+00  -7.976e-05  -7.976e-05   8.577e-01   1.144e+00  

Coefficients:
              Estimate Std. Error z value Pr(>|z|)
(Intercept)    0.08004    0.40032   0.200    0.842
treatment2   -19.64611 2150.80263  -0.009    0.993
...
\begin{Schunk}
\begin{Sinput}
> HPDinterval(mcmc(predict.MCMCglmm(m2c.4$Sol[, 2], 1, "binomial")))
\end{Sinput}
\begin{Soutput}
           lower     upper
var1 0.000711048 0.1060524
attr(,"Probability")
[1] 0.95
\end{Soutput}
\end{Schunk}
